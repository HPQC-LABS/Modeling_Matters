...
## Hartree-Fock:
#####################
mf = mol.RHF().set(conv_tol=1e-10,max_cycle=999,direct_scf_tol=1e-14,chkfile = 'out_1e-2.chk').run()
...
for sym in ['A1','A2','B1','B2']:
    mci = fci.FCI(mol, mf.mo_coeff, singlet=False).set(conv_tol=1e-2,max_cycle=999,wfnsym = sym)
