#!/bin/bash
#SBATCH --job-name=FeCO4-C3v-S0.sh
#SBATCH --time=336:00:00
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=32
#SBATCH --mem=32G
#SBATCH --output=FeCO4-C3v-S0.out
#SBATCH --error=FeCO4-C3v-S0.err
...
cd $CFOURSCRATCH/
echo "Attempting to make FeCO4-C3v-S020250305091610 in $CFOURSCRATCH/...  TIME: $(date +%Ec)"

mkdir FeCO4-C3v-S020250305091610
if [ -e FeCO4-C3v-S020250305091610/ ]; then
	echo "Success! TIME: $(date +%Ec)"
else
...
xclean
xcfour > FeCO4-C3v-S0.out
echo "CFour finished! TIME: $(date +%Ec)"

echo "Attempting to copy back files from $CFOURSCRATCH... TIME: $(date +%Ec)"
cp FeCO4-C3v-S0.out $JOBPATH/FeCO4-C3v-S0.out
cp MOLDEN $JOBPATH/FeCO4-C3v-S0.molden
cp QUADRATURE $JOBPATH/FeCO4-C3v-S0.quadrature

echo "Attempting to clean $CFOURSCRATCH... save is False. TIME: $(date +%Ec)"
cd ../
rm -rf FeCO4-C3v-S020250305091610
